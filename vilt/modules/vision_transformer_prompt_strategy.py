...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, prompts=None, learnt_p=False, prompt_type='head'):
        B, N, C = x.shape
        if prompts is None or prompt_type in ('head','cross') :
            # origin attention
            qkv = (
                self.qkv(x)
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, prompts=None, learnt_p=False, prompt_type='head'):
        text_embs,img_embs = x
        if prompts is not None:
            text_prompt,img_prompt = prompts
            text_embs_len,text_prompt_len,img_embs_len,img_prompt_len = text_embs.shape[1],text_prompt.shape[1],img_embs.shape[1],img_prompt.shape[1]
            if prompt_type == 'head':  
                x = torch.cat([text_prompt+img_prompt,text_embs,img_embs],dim=1) 
            elif prompt_type == 'cross':
                x = torch.cat([text_prompt,text_embs,img_prompt,img_embs],dim=1)    
        else:
            text_embs_len,text_prompt_len,img_embs_len,img_prompt_len = text_embs.shape[1],0,img_embs.shape[1],0
            x = torch.cat([text_embs,img_embs],dim=1)    
            
        _x, attn = self.attn(self.norm1(x), mask=mask, prompts=prompts, learnt_p=learnt_p, prompt_type=prompt_type)
        x = x + self.drop_path(_x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x[:,0:text_embs_len+text_prompt_len,:],x[:,text_embs_len+text_prompt_len:,:]
