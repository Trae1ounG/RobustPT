...
def kl_divergence(p, q):
    loss = F.kl_div(p.softmax(-1).log(), q.softmax(-1), reduction='sum')
    return loss

def compute_distance(pl_module,batch,is_text=True):
    infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    text_prompts = infer["text_prompts"]
    image_prompts = infer["image_prompts"]
    distance_loss = kl_divergence(text_prompts, image_prompts)
    if is_text:
        distance_loss = distance_loss * 0.4
    else:
        distance_loss = distance_loss * 0.3
    ret = {
        "distance_loss": distance_loss
    }
    return ret
       
def compute_mlm(pl_module, batch,step_cnt):
    infer = pl_module.infer(batch, mask_text=True, mask_image=False,step_num=step_cnt)
...
def compute_mpp(pl_module, batch, step_cnt):
    infer = pl_module.infer(batch, mask_text=True, mask_image=False,step_num=step_cnt)
...
def compute_mppd(pl_module, batch, step_cnt):
    infer = pl_module.infer(batch, mask_text=False, mask_image=True, step_num=step_cnt)
...
def compute_mpfr(pl_module, batch, step_cnt):
    infer = pl_module.infer(batch, mask_text=False, mask_image=True, step_num=step_cnt)
...
def compute_itm_wpa(pl_module, batch, step_cnt):
    ...
    infer = pl_module.infer(batch, mask_text=False, mask_image=False, step_num=step_cnt)
...
def compute_mmimdb(pl_module, batch, step_cnt):
    phase = "train" if pl_module.training else "val"
    if phase == "train":
        infer = pl_module.infer(batch, mask_text=False, mask_image=False, step_num=step_cnt)
    else:
...
def compute_hatememes(pl_module, batch, step_cnt):
    phase = "train" if pl_module.training else "val"
    if phase == "train":
        infer = pl_module.infer(batch, mask_text=False, mask_image=False, step_num=step_cnt)
    else:
...
def compute_food101(pl_module, batch, step_cnt):
    phase = "train" if pl_module.training else "val"
    if phase == "train":
        infer = pl_module.infer(batch, mask_text=False, mask_image=False, step_num=step_cnt)
    else:
